...
def one_IM(array_one, selected_dataset, folder):
...
def two_IM(array_one, array_two, selected_dataset, folder):
    if selected_dataset == 1:
        if type(array_one[0]) is list:
            for threshold in np.arange(0.0, 1.01, 0.01):
                for j in range(0, len(window_sizes), 1):
...
def all_IM(array_one, array_two, array_three, selected_dataset, folder):
    if selected_dataset == 1:
        for threshold in np.arange(0.0, 1.01, 0.01):
            # print "threshold = ", threshold
            for j in range(0, len(window_sizes), 1):
